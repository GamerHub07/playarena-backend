...
export class LudoHandler extends BaseHandler {
    ...
    private async handleGameAction(socket: Socket, payload: GameActionPayload): Promise<void> {
        ...
        if (!engine) {
            // Game not found - might be handled by another handler
            return;
        }

        // Only handle Ludo games - skip chess and other game types
        if (engine.getGameType() !== 'ludo') {
            return; // Let ChessHandler or other handlers deal with it
        }
